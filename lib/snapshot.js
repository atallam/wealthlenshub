...
export async function takeSnapshot(userId, { source = "manual", cas_statement_date = null } = {}) {
  ...
  const SEL = "id, member_id, type, units, purchase_price, current_price, purchase_value, current_value, currency";
  // Exclude soft-exited CAS rows (migration 0029); fall back if the column is missing.
  let { data: holdings, error: hErr } = await supabase.from("holdings").select(SEL).eq("user_id", userId)
    .or("holding_status.is.null,holding_status.neq.exited");
  if (hErr) ({ data: holdings } = await supabase.from("holdings").select(SEL).eq("user_id", userId));
