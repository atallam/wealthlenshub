...
export function parseNSDLCASStatement(rawText) {
  ...
  while((infMatch=infRe.exec(rawText))!==null) {
    ...
    // ── NSDL CAS tabular MF: first line has ≥4 tab-delimited cols (same as INE demat) ──
    // cols[0]=Name Part 1, cols[1]=units, cols[4]="nav value" space-separated
    ...
    const _nav=_pvNums[0]||0,_val=_pvNums[1]||0;
    if(_rn&&_val>0){const _name=cleanCASName(_rn);
    const _h={name:_name,type:"MF",ticker:isin,scheme_code:"",units:_units,purchase_nav:_nav,current_nav:_nav,purchase_price:_nav,current_price:_nav,purchase_value:_val,current_value:_val,source:_source,brokerage_name:_brokerage,currency:"INR",_folio:"",_needs_price:true};
