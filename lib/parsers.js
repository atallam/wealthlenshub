...
function parseAxisCSV(rows, h, dataRows) {
  // Header-driven column lookup — Axis exports vary (some add a leading Sr.No.
  // column, some reorder Chq/Ref), so hardcoded positions silently broke when
  // the layout shifted. Fall back to the historical fixed positions only if
  // the header row doesn't give us a match.
  const di = h.findIndex(c=>c.includes("tran date")||c.includes("value date")||c==="date");
  const descI = h.findIndex(c=>c.includes("particular"));
  ...
  const refI = h.findIndex(c=>c.includes("chq")||c.includes("cheque")||c.includes("ref"));
  return dataRows.map(r=>({
    date: r[di>=0?di:0],
    desc: r[descI>=0?descI:2],
    debit: r[debI>=0?debI:3],
    credit: r[credI>=0?credI:4],
    balance: r[balI>=0?balI:5],
    ref: r[refI>=0?refI:1],
