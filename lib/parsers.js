...
function genericCSV(rows, statementType) {
...
export function parseCSV(text, bankKey, statementType) {
  ...
  if(!headerRow&&!bankKey) return {rows:genericCSV(rows,statementType),detectedBank:null};
  ...
  if(bankKey&&bankKey!=="auto"&&bankKey!=="other_us"&&bankKey!=="other_in"&&BANK_COLUMN_MAP[bankKey])
    return {rows:parseBankCSV(rows,h,dataRows,bankKey,statementType), detectedBank:bankKey};
  const detected = autoDetectBank(h);
  if(detected&&BANK_COLUMN_MAP[detected]) return {rows:parseBankCSV(rows,h,dataRows,detected,statementType), detectedBank:detected};
  return {rows:genericCSV(rows,statementType), detectedBank:null};
}
