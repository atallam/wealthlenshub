...
export function parseNSDLCASStatement(rawText) {
  ...
  while((infMatch=infRe.exec(rawText))!==null) {
    ...
    console.log(`[MF_TAB] ${isin} cols=${_bCols.length} c1=${_bCols[1]} c2=${_bCols[2]} c3=${_bCols[3]} c4=${_bCols[4]}`);
    const _pvNums=[...(_bCols[4]||"").matchAll(/-?[\d,]+\.?\d*/g)].map(m=>pNum(m[0])).filter(n=>n>0);
    let _nav=0,_val=0,_purchaseNav=0;
    if(_pvNums.length>=2){// Layout A: cols[4] = "nav value"
      _nav=_pvNums[0];_val=_pvNums[1];_purchaseNav=_nav;}
    ...
    console.log(`[MF_TAB] ${isin} units=${_units} nav=${_nav} purchaseNav=${_purchaseNav} val=${_val}`);
