...
export function parseNSDLCASStatement(rawText) {
  ...
  const _fsS=_fsM.index;const _fsEm=rawText.substring(_fsS).match(/\b(?:Sub[\s-]*Total|Grand[\s-]*Total)\b/i);
  const _fsT=rawText.substring(_fsS,_fsEm?_fsS+_fsEm.index:Math.min(_fsS+100000,rawText.length));
