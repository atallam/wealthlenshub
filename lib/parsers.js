...
export function parseNSDLCASStatement(rawText) {
  ...
  try {
    const re2 = /\b(IN[FE][A-Z0-9]{9})\b/g; let m2;
    console.log(`[CAS_DBG] rawText.length=${rawText.length}`);
    console.log(`[CAS_DBG] FULL_TEXT_START>>>${rawText.slice(0,3000)}<<<FULL_TEXT_END`);
    while ((m2 = re2.exec(rawText)) !== null) {
      const before = rawText.slice(Math.max(0,m2.index-200),m2.index).replace(/\n/g,"↵");
      const after  = rawText.slice(m2.index, m2.index+800).replace(/\n/g,"↵");
      console.log(`[CAS_DBG_ISIN] ${m2[1]} pos=${m2.index}`);
      console.log(`[CAS_DBG_BEFORE] ${before}`);
      console.log(`[CAS_DBG_AFTER] ${after}`);
    }
  } catch(e) { console.log("[CAS_DBG] err:", e.message); }
  // ── END DEBUG LOG ─────────────────────────────────────────────────────────────
  const holdings=[], warnings=[];
  ...
  while((ineMatch=ineRe.exec(rawText))!==null) {
    ...
    const tickerInRawName=extractExchangeTicker(rawName);
    const exchangeTicker=tickerFromSecondLine||tickerInRawName||extractExchangeTicker(before.split(/\n/).pop()||"");
    let name=cleanCASName(rawName);
    // cleanCASName strips the "ACE.NSE " prefix but leaves the face-value number that
    // NSDL CAS PDFs place between the exchange code and the security name text
    // (e.g., "ACE.NSE 75.000 EQUIPMENTS LIMITED" → "75.000 EQUIPMENTS LIMITED").
    // Strip the leading decimal number to recover the security name portion.
    name=name.replace(/^\d+\.\d+\s+(?=[A-Z])/,"").trim();
    // NSDL CAS often omits the ticker word from the security name column because it
    // already appears as the exchange code. Re-prepend it so users see a recognisable name
    // (e.g., "EQUIPMENTS LIMITED -" → "ACE EQUIPMENTS LIMITED -").
    if(tickerInRawName&&name.length>0&&!name.toUpperCase().startsWith(tickerInRawName))
      name=tickerInRawName+" "+name;
  ...
  // Log raw block for each missed ISIN — helps diagnose layout variations
