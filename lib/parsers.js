...
export const BANK_COLUMN_MAP = {
  ...
  axis: {
    ...
    date: ["tran date", "transaction date", "value date", "date"], desc: ["particular"], debit: ["debit", "withdrawal"], credit: ["credit", "deposit"], balance: ["balance"], ref: ["chq", "cheque", "ref"],
...
export function parseBankCSV(rows, h, dataRows, bankKey, statementType) {
  ...
  const fb = cfg.fallbackIdx || {};
  if (di < 0 && fb.date !== undefined) di = fb.date;
  if (descI < 0 && fb.desc !== undefined) descI = fb.desc;
  if (debI < 0 && cfg.debit && fb.debit !== undefined) debI = fb.debit;
  if (credI < 0 && cfg.credit && fb.credit !== undefined) credI = fb.credit;
...
export function parseCSV(text, bankKey, statementType) {
  ...
  const dataRows = headerRow ? rows.slice(rows.indexOf(headerRow)+1) : rows;
