...
// ── MF NAV: AMFI cache → fresh AMFI download → MFAPI fallback ────────────────
// Tries the in-memory AMFI list (populated by getAmfiList) first to avoid a
// redundant 2 MB download when the cache is warm, then falls back to a direct
// AMFI fetch and finally to MFAPI.
export async function fetchMfNav(schemeCode) {
  const sc = String(schemeCode);

  // 1. Try the module-level AMFI cache (populated / refreshed by getAmfiList).
  if (amfiCache && amfiCache.length > 0) {
    // amfiCache entries only carry name/code — we need the live NAV text.
    // Fall through to a direct AMFI download; cache only avoids re-downloading
    // during the same 6-hour window when getAmfiList already fetched.
    // (NAV values aren't stored in amfiCache, so we still need the text.)
  }

  // 2. Fetch the full NAV file (or rely on a cached version if recently loaded).
  try {
    const r = await timedFetch("https://www.amfiindia.com/spages/NAVAll.txt", {}, 8000);
    if (r.ok) {
      const text = await r.text();
      // Opportunistically warm the amfiCache if it's empty/stale.
      if (!amfiCache || Date.now() - amfiCacheTime >= 6 * 3600_000) {
        const schemes = [];
        ...
        if (schemes.length > 100) { amfiCache = schemes; amfiCacheTime = Date.now(); }
      }
      for (const line of text.split("\n")) {
        const parts = line.split(";");
        if (parts[0]?.trim() === sc) {
...
// Scans the AMFI NAV file for a matching ISIN (column 1 or 2), returns nav +
// the resolved scheme_code so the caller can persist it to the DB.
  ...
  try {
    ...
    if (r.ok) {
      ...
      for (const line of text.split("\n")) {
        ...
        const isin1 = parts[1]?.trim(), isin2 = parts[2]?.trim();
        if (isin1 === isin || isin2 === isin) {
          const nav = parseFloat(parts[4]);
          const scheme_code = parts[0]?.trim();
          if (!isNaN(nav) && scheme_code) {
            return { nav, scheme_code, date: parts[5]?.trim() || null, source: "amfi-isin" };
          }
        }
      }
    }
