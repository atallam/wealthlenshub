import { createRequire } from "module";
export const TWELVE_KEY = process.env.TWELVE_DATA_KEY || "";
export const FX_FALLBACK = 94.5;

// ── Generic fetch with timeout ────────────────────────────────────────────────
export async function timedFetch(url, opts = {}, ms = 6000) {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), ms);
  try {
    const r = await fetch(url, { ...opts, signal: ctrl.signal });
    clearTimeout(timer);
    return r;
  } catch (e) {
    clearTimeout(timer);
    throw e;
  }
}

// ── Yahoo Finance helpers ─────────────────────────────────────────────────────
const YAHOO_UA = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36";

export async function yahooFetch(path) {
  for (const host of ["query1.finance.yahoo.com", "query2.finance.yahoo.com"]) {
    try {
      const r = await timedFetch(`https://${host}${path}`, { headers: { "User-Agent": YAHOO_UA, "Accept": "application/json" } });
      if (!r.ok) continue;
      const data = await r.json();
      if (data?.finance?.error?.code === "Too Many Requests") continue;
      return data;
    } catch { continue; }
  }
  return null;
}

export async function yahooChart(symbol) {
  const data = await yahooFetch(`/v8/finance/chart/${encodeURIComponent(symbol)}?interval=1d&range=5d`);
  return data?.chart?.result?.[0]?.meta || null;
}

export async function yahooSearch(q, count = 25) {
  const data = await yahooFetch(`/v1/finance/search?q=${encodeURIComponent(q)}&quotesCount=${count}&newsCount=0&enableFuzzyQuery=true`);
  return data?.quotes || [];
}

export async function yahooPrice(symbol) {
  const meta = await yahooChart(symbol);
  return meta?.regularMarketPrice ?? meta?.chartPreviousClose ?? null;
}

// ── Twelve Data helpers ───────────────────────────────────────────────────────
export async function twelveSearch(q, exchange = "") {
  if (!TWELVE_KEY) return null;
  try {
    const url = `https://api.twelvedata.com/symbol_search?symbol=${encodeURIComponent(q)}&outputsize=20${exchange ? "&exchange=" + exchange : ""}`;
    const r = await timedFetch(url, { headers: { "Authorization": `apikey ${TWELVE_KEY}` } });
    if (!r.ok) return null;
    const data = await r.json();
    if (data.status === "error") return null;
    return data.data || [];
  } catch { return null; }
}

export async function twelveQuote(symbol) {
  if (!TWELVE_KEY) return null;
  try {
    const url = `https://api.twelvedata.com/quote?symbol=${encodeURIComponent(symbol)}&apikey=${TWELVE_KEY}`;
    const r = await timedFetch(url);
    if (!r.ok) return null;
    const data = await r.json();
    if (data.status === "error" || !data.close) return null;
    return {
      name:     data.name || symbol,
      price:    parseFloat(data.close),
      exchange: data.exchange || "",
      currency: data.currency || "",
    };
  } catch { return null; }
}

// ── Stock price: Twelve Data → Yahoo ─────────────────────────────────────────
export async function stockPrice(symbol, marketSuffix = "") {
  const tdSymbol = marketSuffix ? symbol.replace(/\.(NS|BO)$/, "") : symbol;
  if (TWELVE_KEY) {
    const q = await twelveQuote(tdSymbol + (marketSuffix ? `:${marketSuffix}` : ""));
    if (q?.price) return q;
  }
  const meta = await yahooChart(symbol);
  if (!meta) return null;
  const price = meta.regularMarketPrice ?? meta.chartPreviousClose;
  if (!price) return null;
  return { name: meta.longName || meta.shortName || "", price, exchange: meta.exchangeName || "", currency: meta.currency || "" };
}

// ── ISIN → Yahoo symbol (Indian equities) ─────────────────────────────────────
// CAS imports store the ISIN (INE…) as the ticker; Yahoo/Twelve Data need the
// exchange symbol (RELIANCE.NS). Seed map lives in lib/isin_symbols.json and is
// extended at runtime via Yahoo search. Prefer NSE over BSE.
const _require = createRequire(import.meta.url);
let ISIN_SEED = {};
try { ISIN_SEED = _require("./isin_symbols.json"); } catch { ISIN_SEED = {}; }
const isinSymbolCache = new Map(Object.entries(ISIN_SEED));

export function isIsin(t) { return /^IN[EF][0-9A-Z]{9}$/i.test(String(t || "").trim()); }

export async function resolveIsinSymbol(isin) {
  const key = String(isin || "").trim().toUpperCase();
  if (!isIsin(key)) return null;
  if (isinSymbolCache.has(key)) return isinSymbolCache.get(key);
  const quotes = await yahooSearch(key, 8);
  const pick = quotes.find(q => q.symbol?.endsWith(".NS")) || quotes.find(q => q.symbol?.endsWith(".BO"));
  const sym = pick?.symbol || null;
  if (sym) isinSymbolCache.set(key, sym);
  return sym;
}

/** Price for an Indian equity holding whose ticker may be an NSE symbol or an ISIN. */
export async function indianStockPrice(ticker) {
  const t = String(ticker || "").trim().toUpperCase();
  if (!t) return null;
  if (isIsin(t)) {
    const sym = await resolveIsinSymbol(t);
    if (!sym) return null;
    const suffix = sym.endsWith(".BO") ? "BSE" : "NSE";
    const q = await stockPrice(sym, suffix);
    return q?.price ?? await yahooPrice(sym);
  }
  const q = await stockPrice(`${t}.NS`, "NSE");
  return q?.price ?? await yahooPrice(`${t}.BO`);
}

// ── Stock search: Twelve Data → Yahoo ────────────────────────────────────────
export async function stockSearch(q, market = "US") {
  if (TWELVE_KEY) {
    const exchange = market === "IN" ? "NSE,BSE" : "";
    const results = await twelveSearch(q, exchange);
    if (results?.length) {
      return results
        .filter(s => market === "IN"
          ? ["NSE","BSE"].includes(s.exchange)
          : !["NSE","BSE"].includes(s.exchange) && s.country === "United States")
        .filter(s => ["Common Stock","ETF","Equity"].includes(s.instrument_type || s.type))
        .slice(0, 12)
        .map(s => ({
          ticker:   s.symbol,
          symbol:   s.symbol,
          name:     s.instrument_name || s.symbol,
          exchange: s.exchange || "",
          type:     s.instrument_type || s.type || "EQUITY",
        }));
    }
  }
  const quotes = await yahooSearch(q);
  const ETF_KW = /etf|bees|fund|gold|nifty|sensex|index|midcap|smallcap|liquid|overnight|banking|silver|copper|nasdaq/i;
  return quotes
    .filter(qt => {
      if (market === "IN") {
        const onIN = qt.symbol?.endsWith(".NS") || qt.symbol?.endsWith(".BO") || qt.exchange === "NSI" || qt.exchange === "BSE";
        return onIN && (qt.quoteType === "EQUITY" || qt.quoteType === "ETF" || ETF_KW.test(qt.longname || ""));
      }
      const notIN = !qt.symbol?.endsWith(".NS") && !qt.symbol?.endsWith(".BO") && qt.exchange !== "NSI" && qt.exchange !== "BSE";
      return notIN && (qt.quoteType === "EQUITY" || qt.quoteType === "ETF");
    })
    .slice(0, 12)
    .map(qt => ({
      ticker:   market === "IN" ? (qt.symbol?.replace(/\.(NS|BO)$/, "") || qt.symbol) : qt.symbol,
      symbol:   qt.symbol,
      name:     qt.longname || qt.shortname || qt.symbol,
      exchange: qt.exchange || "",
      type:     qt.quoteType || "EQUITY",
    }));
}

// ── AMFI NAV cache — single shared cache for all three callers ────────────────
// navTextCache holds the raw NAVAll.txt content (downloaded once, refreshed every 6h).
// navMap is built from it: Map<schemeCode, {nav, date}> for O(1) lookup.
// schemeCache holds the scheme list (code/name/isin) for search.
let navTextCache = null;
let navTextCacheTime = 0;
let navMap = new Map();          // schemeCode → { nav, date }
let isinMap = new Map();         // isin → { nav, scheme_code, date }
let amfiCache = null;            // scheme list for search
let amfiCacheTime = 0;

const AMFI_TTL = 6 * 3600_000;  // 6 hours
let amfiRefreshPromise = null;  // singleton guard — prevents concurrent duplicate downloads

async function refreshAmfiCache() {
  if (navTextCache && Date.now() - navTextCacheTime < AMFI_TTL) return; // already fresh
  if (!amfiRefreshPromise) {
    amfiRefreshPromise = _doRefreshAmfi().finally(() => { amfiRefreshPromise = null; });
  }
  return amfiRefreshPromise;
}

async function _doRefreshAmfi() {
  try {
    // AMFI moved the file to portal.amfiindia.com (www.* now 302s). Try portal first.
    let r = null;
    for (const url of ["https://portal.amfiindia.com/spages/NAVAll.txt", "https://www.amfiindia.com/spages/NAVAll.txt"]) {
      try { r = await timedFetch(url, {}, 15000); if (r.ok) break; } catch { r = null; }
    }
    if (!r) return;
    if (!r.ok) return;
    const text = await r.text();
    const schemes = [], newNavMap = new Map(), newIsinMap = new Map();
    for (const line of text.split("\n")) {
      const parts = line.split(";");
      if (parts.length < 6 || !/^\d+$/.test(parts[0]?.trim())) continue;
      // AMFI format (2026): Code;ISIN1;ISIN2;Scheme Name;Plan;Option;NAV;Date  (8 fields)
      // Legacy format:      Code;ISIN1;ISIN2;Scheme Name;NAV;Date              (6 fields)
      // NAV and Date are always the last two fields — read from the end so both formats work.
      const sc    = parts[0].trim();
      const isin1 = parts[1]?.trim() || "";
      const isin2 = parts[2]?.trim() || "";
      const name  = parts.slice(3, parts.length - 2).map(x => x.trim()).filter(Boolean).join(" - ");
      const nav   = parseFloat(parts[parts.length - 2]);
      const date  = parts[parts.length - 1]?.trim() || null;
      if (!isNaN(nav)) {
        newNavMap.set(sc, { nav, date });
        if (isin1.startsWith("INF")) newIsinMap.set(isin1, { nav, scheme_code: sc, date });
        if (isin2.startsWith("INF")) newIsinMap.set(isin2, { nav, scheme_code: sc, date });
      }
      schemes.push({ scheme_code: sc, isin1, isin2, name });
    }
    if (schemes.length > 100) {
      navTextCache    = text;
      navTextCacheTime = Date.now();
      navMap          = newNavMap;
      isinMap         = newIsinMap;
      amfiCache       = schemes;
      amfiCacheTime   = Date.now();
      console.log(`📋 AMFI cache refreshed: ${schemes.length} schemes, ${newNavMap.size} NAVs`);
    }
  } catch (e) {
    console.warn("AMFI refresh failed:", e.message);
  }
}

// ── MF NAV by scheme code ─────────────────────────────────────────────────────
export async function fetchMfNav(schemeCode) {
  const sc = String(schemeCode);
  await refreshAmfiCache();
  const cached = navMap.get(sc);
  if (cached) return { nav: cached.nav, date: cached.date, meta: { fund_house: "" }, source: "amfi" };

  // MFAPI fallback
  try {
    const r = await timedFetch(`https://api.mfapi.in/mf/${sc}`, {}, 5000);
    if (r.ok) {
      const data = await r.json();
      const nav = parseFloat(data?.data?.[0]?.nav);
      if (!isNaN(nav)) return { nav, date: data?.data?.[0]?.date || null, meta: data?.meta || {}, source: "mfapi" };
    }
  } catch { /* fall through */ }
  return null;
}

// ── MF NAV by ISIN — for CAS holdings where scheme_code lookup failed ─────────
export async function fetchMfNavByIsin(isin) {
  if (!isin || !isin.startsWith("INF")) return null;
  await refreshAmfiCache();
  const cached = isinMap.get(isin);
  if (cached) return { ...cached, source: "amfi-isin" };
  return null;
}

// ── Historical NAV on / near a date (mfapi.in history) ───────────────────────
// Used by the ledger inference step to price an inferred purchase on its
// likely date. Searches ±maxOffset days for the nearest published NAV.
const navHistCache = new Map();   // schemeCode → { at, map: Map<isoDate, nav> }
export async function fetchMfNavOn(schemeCode, isoDate, maxOffset = 7) {
  const sc = String(schemeCode || "").trim();
  if (!sc || !isoDate) return null;
  let hist = navHistCache.get(sc);
  if (!hist || Date.now() - hist.at > 6 * 3600 * 1000) {
    try {
      const r = await timedFetch(`https://api.mfapi.in/mf/${sc}`, {}, 8000);
      if (!r.ok) return null;
      const data = await r.json();
      const map = new Map();
      for (const e of data?.data || []) {
        const [dd, mm, yyyy] = String(e.date).split("-");
        if (yyyy) map.set(`${yyyy}-${mm}-${dd}`, parseFloat(e.nav));
      }
      hist = { at: Date.now(), map };
      navHistCache.set(sc, hist);
    } catch { return null; }
  }
  const base = new Date(isoDate + "T00:00:00Z");
  for (let off = 0; off <= maxOffset; off++) {
    for (const sign of off === 0 ? [0] : [-1, 1]) {
      const d = new Date(base); d.setUTCDate(d.getUTCDate() + sign * off);
      const key = d.toISOString().slice(0, 10);
      const nav = hist.map.get(key);
      if (nav && !isNaN(nav)) return { nav, date: key, offset: sign * off };
    }
  }
  return null;
}

/** ISIN → AMFI scheme code (from the cached AMFI master list). */
export async function schemeCodeForIsin(isin) {
  if (!isin || !String(isin).startsWith("INF")) return null;
  await refreshAmfiCache();
  return isinMap.get(String(isin).trim().toUpperCase())?.scheme_code || null;
}

// ── AMFI master list (for MF search) ─────────────────────────────────────────
export async function getAmfiList() {
  await refreshAmfiCache();
  if (amfiCache?.length) return amfiCache;
  // Last-resort fallback: MFAPI scheme list (no NAVs, just names)
  try {
    const r = await timedFetch("https://api.mfapi.in/mf", {}, 10000);
    if (r.ok) {
      const data = await r.json();
      amfiCache = (data || []).map(f => ({ scheme_code: String(f.schemeCode), name: f.schemeName }));
      amfiCacheTime = Date.now();
      console.log(`📋 AMFI list fallback from MFAPI: ${amfiCache.length} schemes`);
    }
  } catch { /* keep empty */ }
  return amfiCache || [];
}

export function scoreMf(name, qLower, qWords) {
  const n = name.toLowerCase();
  if (n.startsWith(qLower)) return 100;
  if (n.includes(qLower)) return 80;
  const hits = qWords.filter(w => n.includes(w)).length;
  if (hits === qWords.length) return 60;
  return hits > 0 ? Math.round(40 * hits / qWords.length) : 0;
}

// ── FX rates ──────────────────────────────────────────────────────────────────
export const FX_CACHE = { rates: {}, ts: 0 };
export const FX_FALLBACKS = { INR: 94.5, EUR: 0.88, GBP: 0.76, SGD: 1.30, AED: 3.67, AUD: 1.50, JPY: 150.0, CAD: 1.38, CHF: 0.85 };

export async function fetchAllFxRates() {
  if (Date.now() - FX_CACHE.ts < 600_000 && Object.keys(FX_CACHE.rates).length > 0) return FX_CACHE.rates;
  try {
    const r = await timedFetch("https://open.er-api.com/v6/latest/USD", {}, 5000);
    if (r.ok) {
      const data = await r.json();
      if (data?.rates) { FX_CACHE.rates = data.rates; FX_CACHE.ts = Date.now(); return data.rates; }
    }
  } catch { /* fall through */ }
  return FX_FALLBACKS;
}

export async function fetchUsdInr() {
  const rates = await fetchAllFxRates();
  const rate = rates?.INR;
  if (rate && rate > 50 && rate < 200) return { rate, source: FX_CACHE.ts > 0 ? "exchangerate-api" : "fallback" };
  try {
    const yRate = await yahooPrice("USDINR=X");
    if (yRate && yRate > 50 && yRate < 200) return { rate: yRate, source: "yahoo" };
  } catch { /* fall through */ }
  return { rate: FX_FALLBACK, source: "hardcoded" };
}

export async function mfNav(schemeCode) {
  const result = await fetchMfNav(schemeCode);
  return result?.nav ?? null;
}
