 ...
 */
import { getMasked, MASK } from '../../utils.js';
...
export default function DonutChart({data, total, AT}){
  ...
  const fmtV=v=>getMasked()?MASK:v>=1e7?`₹${(v/1e7).toFixed(2)}Cr`:v>=1e5?`₹${(v/1e5).toFixed(2)}L`:`₹${Math.round(v).toLocaleString("en-IN")}`;
  return(
    <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"2.5rem",flexWrap:"wrap",padding:".5rem 0"}}>
      <svg width={S} height={S} viewBox={`0 0 ${S} ${S}`}>
        ...
        <text x={cx} y={cy-6} textAnchor="middle" fill="#0F3D38" fontSize="11" fontFamily="JetBrains Mono,DM Mono,monospace" fontWeight="700">{getMasked()?MASK:total>=1e7?(total/1e7).toFixed(1)+"Cr":total>=1e5?(total/1e5).toFixed(1)+"L":"₹"+Math.round(total)}</text>
