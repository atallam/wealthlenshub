    ...
    <Overlay onClose={onClose} wide>
      ...
      {step==="upload"&&(<>
        <div style={{border:dragOver?"2px dashed #c9a84c":"2px dashed var(--border-mid)",borderRadius:12,
          ...
          <div style={{fontSize:".72rem",color:"var(--text)",marginTop:".4rem"}}>
        ...
        <div style={{marginBottom:".8rem"}}>
          <div style={{display:"flex",alignItems:"center",gap:".5rem",marginBottom:".45rem"}}>
            <span style={{fontSize:".6rem",color:"var(--text)",letterSpacing:".06em",textTransform:"uppercase",minWidth:"1.8rem"}}>US</span>
            <div style={{display:"flex",flexWrap:"wrap",gap:".35rem"}}>
              {US_FORMATS.map(f=>(
                <span key={f.name} style={{fontSize:".67rem",padding:".22rem .55rem",borderRadius:4,
                  background:"var(--bg-muted)",color:"var(--text)",border:"1px solid var(--border)",borderLeft:`3px solid ${f.color}`,whiteSpace:"nowrap"}}>
                  {f.name}
                </span>
              ))}
            </div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:".5rem",marginBottom:".45rem"}}>
            <span style={{fontSize:".6rem",color:"var(--text)",letterSpacing:".06em",textTransform:"uppercase",minWidth:"1.8rem"}}>IN</span>
            <div style={{display:"flex",flexWrap:"wrap",gap:".35rem"}}>
              {IN_FORMATS.map(f=>(
                <span key={f.name} style={{fontSize:".67rem",padding:".22rem .55rem",borderRadius:4,
                  background:"var(--bg-muted)",color:"var(--text)",border:"1px solid var(--border)",borderLeft:`3px solid ${f.color}`,whiteSpace:"nowrap"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:".5rem"}}>
            <span style={{fontSize:".6rem",color:"var(--text)",minWidth:"1.8rem"}}></span>
            <div style={{display:"flex",flexWrap:"wrap",gap:".35rem"}}>
              {OTHER_FORMATS.map(f=>(
                <span key={f.name} style={{fontSize:".67rem",padding:".22rem .55rem",borderRadius:4,
                  background:"var(--bg-muted)",color:"var(--text)",border:"1px solid var(--border)",borderLeft:`3px solid ${f.color}`,whiteSpace:"nowrap"}}>
      ...
      {step==="cas_password"&&(
        <div style={{maxWidth:400,margin:"0 auto"}}>
          <div style={{textAlign:"center",marginBottom:"1.2rem"}}>
            ...
            <div style={{fontSize:".72rem",color:"var(--text)"}}>NSDL/CDSL CAS password is your PAN number</div>
          </div>
          <div style={{marginBottom:".8rem"}}>
      ...
      {step==="preview"&&(<>
        ...
        {mode==="holdings"&&members.length>0&&(
          <div style={{marginBottom:".8rem"}}>
            {accounts.length>0&&members.length>1?(
              <div>
                <div style={{fontSize:".65rem",letterSpacing:".06em",textTransform:"uppercase",color:"var(--text)",marginBottom:".4rem"}}>Map accounts to family members</div>
                {accounts.map(acct=>{
                  ...
                  <div key={acct} style={{display:"flex",gap:".6rem",alignItems:"center",marginBottom:".35rem"}}>
                    <span style={{fontSize:".72rem",color:"var(--text)",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{acct}</span>
              ...
              <div style={{display:"flex",gap:"1rem",alignItems:"center",flexWrap:"wrap"}}>
                <label style={{display:"flex",alignItems:"center",gap:".4rem",fontSize:".73rem",color:"var(--text)"}}>
                  Assign to:
        ...
        {items.length>200&&<div style={{fontSize:".68rem",color:"var(--text)",textAlign:"center",marginTop:".4rem"}}>Showing first 200 of {items.length} rows</div>}
      ...
      {step==="dup_review"&&(<>
        <div style={{marginBottom:"1rem"}}>
          ...
          <div style={{fontSize:".72rem",color:"var(--text)",lineHeight:1.5}}>
        ...
        <div style={{maxHeight:380,overflowY:"auto",borderRadius:8,border:"1px solid var(--border)"}}>
          {holdings.filter(h=>h._duplicate).map((h,i)=>{
            ...
            return(
            <div key={i} style={{padding:".75rem .85rem",borderBottom:"1px solid var(--border)",
              background:action==="skip"?"var(--bg-muted)":"rgba(90,156,224,.03)",
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:".6rem",marginBottom:".45rem"}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:".78rem",fontWeight:600,color:"var(--text)"}}>{h.name}</div>
                  <div style={{fontSize:".65rem",color:"var(--text)",marginTop:".15rem"}}>
                ...
                <div style={{display:"flex",gap:".3rem",flexShrink:0}}>
                  <button onClick={()=>setImportState(s=>({...s,holdings:s.holdings.map((hh)=>hh===h?{...hh,_dupAction:"update"}:hh)}))}
                    style={{fontSize:".65rem",padding:".22rem .5rem",borderRadius:4,cursor:"pointer",border:"1px solid",transition:"all .15s",fontFamily:"'DM Sans',sans-serif",
                      ...
                      borderColor:action==="update"?"rgba(90,156,224,.45)":"var(--border)",
                      color:action==="update"?"#5a9ce0":"var(--text-muted)"}}>
                    ↻ Update
                  </button>
                  <button onClick={()=>setImportState(s=>({...s,holdings:s.holdings.map((hh)=>hh===h?{...hh,_dupAction:"skip"}:hh)}))}
                    style={{fontSize:".65rem",padding:".22rem .5rem",borderRadius:4,cursor:"pointer",border:"1px solid",transition:"all .15s",fontFamily:"'DM Sans',sans-serif",
                      ...
                      borderColor:action==="skip"?"rgba(224,124,90,.35)":"var(--border)",
                      color:action==="skip"?"#e07c5a":"var(--text-muted)"}}>
                    ⊘ Skip
                  </button>
                </div>
              </div>
        ...
        {holdings.filter(h=>!h._duplicate).length>0&&(
          <div style={{fontSize:".72rem",color:"var(--text)",marginTop:".6rem"}}>
      ...
      {step==="importing"&&(
        <div style={{textAlign:"center",padding:"2.5rem 1rem"}}>
          ...
          <div style={{fontSize:".85rem",color:"var(--text)"}}>Importing {items.length} {mode==="transactions"?"transactions":"holdings"}…</div>
          <div style={{fontSize:".7rem",color:"var(--text)",marginTop:".4rem"}}>Creating entries and first transactions</div>
        </div>
      )}
      ...
      {step==="done"&&result&&(
        <div style={{padding:".5rem 0"}}>
          <div style={{background:"rgba(76,175,154,.08)",border:"1px solid rgba(76,175,154,.2)",borderRadius:10,padding:"1rem 1.2rem",marginBottom:".8rem"}}>
            ...
            {result.inserted_count>0&&<div style={{fontSize:".75rem",color:"var(--text)"}}>+ {result.inserted_count} new holding{result.inserted_count>1?"s":""} added</div>}
            ...
            {result.skipped_count>0&&<div style={{fontSize:".75rem",color:"var(--text)"}}>⊘ {result.skipped_count} duplicate{result.skipped_count>1?"s":""} skipped (kept existing)</div>}
            {result.unmatched_count>0&&(<div style={{fontSize:".75rem",color:"#e07c5a",marginTop:".3rem"}}>
              ...
              <div style={{fontSize:".7rem",color:"var(--text)",marginTop:".2rem"}}>{result.unmatched?.join(", ")}</div>
            </div>)}
