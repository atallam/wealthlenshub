...
import {
  listConnections, getConnection, upsertConnection, insertStatement,
  insertTransactionsBatched, deleteTransactionsByIds, updateConnectionById,
  updateConnectionError, deleteConnection,
...
router.get("/status", auth, async (req, res) => {
  ...
  try {
    const { data: connections, error } = await listConnections(req.user.id);
...
router.post("/exchange", auth, async (req, res) => {
  try {
    ...
    await upsertConnection({ id: connId, user_id: req.user.id, item_id, access_token: encrypt(access_token), institution_id: metadata?.institution?.institution_id || "", institution_name, accounts, status: "active", updated_at: new Date().toISOString() });
...
router.post("/sync/:connectionId", auth, async (req, res) => {
  try {
    const conn = await getConnection(req.user.id, req.params.connectionId);
    ...
    if (added.length > 0) {
      ...
      await insertTransactionsBatched(txns);
    }
    if (removed.length > 0) await deleteTransactionsByIds(removed.map(r => "ptxn_" + r.transaction_id));
    await updateConnectionById(conn.id, { cursor, last_synced: now, status: "active", error_code: null, updated_at: now });
    res.json({ ok: true, added: added.length, modified: modified.length, removed: removed.length });
  } catch (e) {
    await updateConnectionError(req.user.id, req.params.connectionId, { status: "error", error_code: e?.response?.data?.error_code || e.message, updated_at: new Date().toISOString() });
