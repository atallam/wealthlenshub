...
async function extractText(buffer, pw) {
  ...
  for (let i = 1; i <= pdf.numPages; i++) {
    ...
    const sortedRows = [...rows.entries()].sort((a, b) => b[0] - a[0]).map(([, items]) => {
      items.sort((a, b) => a.x - b.x);
      let line = "";
      for (let j = 0; j < items.length; j++) {
        if (j > 0) {
          const gap = items[j].x - items[j-1].x - items[j-1].w;
          if (gap > 15) line += "\t"; else if (gap > 2) line += " ";
        }
        line += items[j].str;
      }
      return line;
...
const [,, pdfPath, password, outPath = "cas_method_a.json"] = process.argv;
if (!pdfPath || !password) { console.error("Usage: node cas_compare_existing.mjs <pdf> <password> [out.json]"); process.exit(1); }
(async () => {
  console.log(`[existing-parser] Reading: ${pdfPath}`);
  const buffer = readFileSync(pdfPath);
  const rawText = await extractText(buffer, password);
  console.log(`[existing-parser] Extracted ${rawText.length} chars`);
  const result = parseNSDLCASStatement(rawText);
  const output = {
    method: "existing-parser", format: result.format, depository: result.depository,
    holder_names: result.holder_names, holder_pans: result.holder_pans,
    statement_date: result.statement_date, period_start: result.period_start, period_end: result.period_end,
    total_holdings: result.holdings.length, warnings: result.warnings,
    holdings: result.holdings.map(h => ({
      name:h.name, type:h.type, ticker:h.ticker, scheme_code:h.scheme_code,
      units:h.units, purchase_nav:h.purchase_nav, current_nav:h.current_nav,
      purchase_price:h.purchase_price, current_price:h.current_price,
      purchase_value:h.purchase_value, current_value:h.current_value,
      source:h.source, brokerage_name:h.brokerage_name, currency:h.currency,
      _folio:h._folio, _needs_price:h._needs_price??false, cost_basis:h.cost_basis??null,
    })),
  };
  writeFileSync(outPath, JSON.stringify(output, null, 2), "utf-8");
  console.log(`[existing-parser] Done — ${result.holdings.length} holding(s) → ${outPath}`);
